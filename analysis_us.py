...
df_evaluated.to_csv('df_evaluated_us.csv', index=False)

past_portfolios = pd.read_csv('past_portfolios_us.csv')
past_portfolios[f"{datetime.now().strftime('%Y-%m-%d')}"] = df_evaluated['ticker']
past_portfolios.to_csv('past_portfolios_us.csv', index=False)