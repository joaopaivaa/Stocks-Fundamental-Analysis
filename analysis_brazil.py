...
df_evaluated.to_csv('gold/df_evaluated_brazil.csv', index=False)

past_portfolios = pd.read_csv('gold/past_portfolios_brazil.csv')
past_portfolios[f"{datetime.now().strftime('%Y-%m-%d')}"] = df_evaluated['ticker']
past_portfolios.to_csv('gold/past_portfolios_brazil.csv', index=False)